...
class MailAgentScanner:
    def __init__(
        ...
        self.email_recipients = [
            r
            for r in [
                options.get(CONF_EMAIL_RECIPIENT_1),
                options.get(CONF_EMAIL_RECIPIENT_2),
            ]
            if r
        ]
        self.notify_services = [
            s
            for s in [
                options.get(CONF_NOTIFY_SERVICE_1),
                options.get(CONF_NOTIFY_SERVICE_2),
            ]
            if s
        ]
    ...
    def _process_single_mail(self, msg):
        ...
        if self.enable_debug:
            LOGGER.info(
                f"Bearbetar mail från {sender}. Bilagor: {len(attachment_paths)}"
            )
        ...
        if self.gemini_api_key:
            try:
                ...
                if ai_data.get("event_found") is True:
                    ...
                    # 2. Skicka notifieringar (Mobil + E-post) - utan bilagor
    ...
    def _send_notifications(self, ai_data, original_subject, attachment_paths):
        ...
        if self.email_service and self.email_recipients:
            ...
            # Inga bilagor skickas med i mailet för att undvika problem
            data_payload = {}

            if self.enable_debug:
                LOGGER.info(
                    f"Skickar e-post via {self.email_service} till {self.email_recipients}"
                )
    ...
    def _call_gemini(self, file_paths, subject, body):
        ...
        for path in file_paths:
            uploaded_files.append(
                client.files.upload(file=path, config={"mime_type": "application/pdf"})
            )

        # Hämta nuvarande tid för att ge Gemini en referenspunkt
        ...
        Din uppgift är att analysera mailet och eventuella bilagor för att hitta bokningar eller möten.

        Regler för datum och tid:
        1. Utgå ALLTID från dagens datum ({now_str}) när du tolkar relativa tidsuttryck (t.ex. "imorgon", "på fredag").
        2. Om ett datum anges utan år (t.ex. "16/12"), måste du välja det år som gör att datumet infaller i framtiden närmast efter idag.
           - Exempel: Om idag är 2025-12-15 och texten säger "16/12", då är året 2025.
           - Exempel: Om idag är 2025-12-28 och texten säger "2/1", då är året 2026.
        ...
        {{
            ...
            "start_time": "YYYY-MM-DD HH:MM:SS (eller null om ingen tid kan fastställas)",
            "location": "Plats",
            "type": "Typ"
        }}
        """
